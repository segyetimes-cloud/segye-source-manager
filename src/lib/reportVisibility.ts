...
// 순서를 명시적으로 보장하는 마스터 배열
// Object.entries 사용 시 키 순서가 런타임에 따라 달라질 수 있어 직접 선언
const VISIBILITY_LIST: ReadonlyArray<readonly [ReportVisibility, VisibilityMeta]> = [
  ['author_only', {
    label: '🔒 최고 보안',
    shortLabel: '최고 보안',
...
// 마스터 배열에서 파생 — Object.entries 캐스트 불필요, 순서 보장
export const VISIBILITY_META: Record<ReportVisibility, VisibilityMeta> =
  Object.fromEntries(VISIBILITY_LIST) as Record<ReportVisibility, VisibilityMeta>

export const VISIBILITY_OPTIONS: ReadonlyArray<{ value: ReportVisibility } & VisibilityMeta> =
  VISIBILITY_LIST.map(([value, meta]) => ({ value, ...meta }))

// 보고서 작성/수정 폼에서 사용할 공개 범위 선택지 (3가지로 단순화)
export const GENERAL_VISIBILITY_OPTIONS: ReadonlyArray<{ value: ReportVisibility; label: string; desc: string }> = [
