...
export default function SourceCopyLogs({ sourceId }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await fetch(`/api/sources/${sourceId}/copy-log`)
  ...
  return (
    <div className="glass-card p-4" style={{ marginTop: '1rem' }}>
      <button
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {logs.length > 0 && (
            <div className="flex items-center gap-2">
              <span style={{ background: 'rgba(255,68,68,0.12)', color: '#C04040', border: '1px solid rgba(255,68,68,0.25)', borderRadius: '6px', padding: '2px 10px', fontSize: '12px', fontWeight: 600 }}>{logs.length}회</span>
          ...
          {!loading && logs.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {logs.map((log, idx) => {
                ...
                return (
                  <div key={log.id} style={{ padding: '9px 12px', background: idx % 2 === 0 ? '#131C2C' : '#0D1520', borderRadius: '7px', border: '1px solid #1A2838' }}>
